...
function groupResultsByProperty(results: Results[], property: keyof Results): Category[] {
  ...
  results.forEach((result) => {
    ...
    if (!val) return;
    const existing = categories.find((c) => c.label === val);
...
function classifySeniority(openings: Results[]): Category[] {
  ...
  openings.forEach((opening) => {
    ...
    if (scores["Lead"] && meta["Lead"] && meta["Lead"].titleHits === 0 && meta["Lead"].descStrong === 0) {
      if (scores["Senior"]) delete scores["Lead"];
      else {
        scores["Mid-level"] = (scores["Mid-level"] || 0) + 1;
        delete scores["Lead"];
      }
...
export function computeBase(results: Results[]) {
  ...
  const cityCategories: Category[] = location
    .map((city) => {
...
export type SlimBase = {
  categories: {
    ...
    frameworks: SlimCategory[];
    databases: SlimCategory[];
    cloud: SlimCategory[];
    devops: SlimCategory[];
    dataScience: SlimCategory[];
    softSkills: SlimCategory[];
    cyberSecurity: SlimCategory[];
    positions: SlimCategory[];
    seniority: SlimCategory[];
    workMode?: SlimCategory[];
    cities?: SlimCategory[];
    salary?: SlimCategory[];
  };
  companies: SlimCategory[];
  locations: SlimCategory[];
};
...
export function computeBaseSlim(results: Results[]): SlimBase {
  ...
  return {
    categories: {
      ...
      frameworks: toSlim(full.categories.frameworks),
      databases: toSlim(full.categories.databases),
      cloud: toSlim(full.categories.cloud),
      devops: toSlim(full.categories.devops),
      dataScience: toSlim(full.categories.dataScience),
      softSkills: toSlim(full.categories.softSkills),
      cyberSecurity: toSlim(full.categories.cyberSecurity),
      positions: toSlim(full.categories.positions),
      seniority: toSlim(full.categories.seniority),
      workMode: toSlim(full.categories.workMode),
      cities: toSlim(full.categories.cities),
      salary: toSlim(full.categories.salary),
    },
    companies: toSlim(full.companies),
    locations: toSlim(full.locations),
  };
}
