...
const WORK_MODE_FI: Record<string, string> = { remote: "remote", hybrid: "hybridi", onsite: "toimisto" };
const SENIORITY_FI: Record<string, string> = { junior: "junior", mid: "mid-level", senior: "senior", lead: "lead" };

function buildSeoHeadline(filters: ReturnType<typeof parseQueryToFilters>): { title: string; description: string } | null {
  if (!filters) return null;
  const techKeys = ["languages", "frameworks", "databases", "cloud", "devops", "dataScience", "cyberSecurity", "roles"];
  const tech = techKeys.flatMap((k) => Array.from(filters[k]?.include ?? []));
  const locations = Array.from(filters["locations"]?.include ?? []);
  const modes = Array.from(filters["workMode"]?.include ?? []).map((m) => WORK_MODE_FI[m] ?? m);
  const levels = Array.from(filters["seniority"]?.include ?? []).map((s) => SENIORITY_FI[s] ?? s);

  if (tech.length === 0 && locations.length === 0 && modes.length === 0 && levels.length === 0) return null;

  const techStr = tech.slice(0, 4).join(", ");
  const locStr = locations.length > 0 ? ` ${locations.slice(0, 2).join("/")}ssa` : " Suomessa";
  const modeStr = modes.length > 0 ? ` · ${modes[0]}` : "";
  const levelStr = levels.length > 0 ? ` · ${levels[0]}` : "";

  const title = `${techStr || "IT"}${levelStr}${modeStr} työpaikat${locStr}`;

  const parts: string[] = [];
  if (tech.length > 0) parts.push(`${tech.slice(0, 3).join(", ")} osaajia`);
  if (levels.length > 0) parts.push(`${levels.join("/")} tasolla`);
  if (modes.length > 0) parts.push(`${modes.join("/")} -työ`);
  if (locations.length > 0) parts.push(`sijainti: ${locations.slice(0, 3).join(", ")}`);
  const description = `Avoimet ${techStr || "IT"}-työpaikat${locStr}. ${parts.join(", ")}. Reaaliaikaiset ilmoitukset Koodaripula.fi:ssä.`;

  return { title, description };
}

function SeoHeadline({ filters }: { filters: ReturnType<typeof parseQueryToFilters> }) {
  const seo = buildSeoHeadline(filters);
  if (!seo) return null;
  return (
    <div className="mb-4 px-1">
      <h2 className="text-base font-semibold text-white leading-snug">{seo.title}</h2>
      <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">{seo.description}</p>
    </div>
  );
}
