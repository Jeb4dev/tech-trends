...
function formatDescription(raw: string, activeQuery?: QueryParams) {
  if (!raw) return "";
  // Preserve blank lines so we can detect double (or more) blank line separators
  const lines = raw.replace(/\r/g, "").split(/\n/); // DO NOT collapse multiple newlines
  const bullets = ["-", "*", "•", "·"]; // bullet tokens
  const blocks: string[] = [];
  let list: string[] = []; // accumulate bullet items
  let blankRun = 0; // count consecutive blank lines

  const activeTerms = gatherActiveTerms(activeQuery);
  activeTerms.sort((a,b)=> b.length - a.length); // prefer longer matches
  const highlightRegex = activeTerms.length ? new RegExp(`(${activeTerms.map(t=>escapeRegExp(t)).join('|')})`, 'gi') : null;

  const applyHighlight = (htmlEscaped: string) => {
    if (!highlightRegex) return htmlEscaped;
    return htmlEscaped.replace(highlightRegex, m => `<mark class='bg-amber-300 text-black rounded px-0.5'>${m}</mark>`);
  };

  function flushList() {
    if (!list.length) return;
    const items = list
      .map(li => `<li class='mb-1'>${applyHighlight(escapeHtml(li))}</li>`)
      .join("\n");
    blocks.push(`<ul class='list-disc list-outside pl-5 my-3 space-y-1'>${items}</ul>`);
    list = [];
  }
  ...
  function looksLikeHeading(line: string) {
    ...
    if (!t) return false;
    return (
      (t.length < 70 && t.endsWith(":")) ||
      (t.length < 60 && /^[A-ZÅÄÖ0-9][A-ZÅÄÖ0-9\s/&+()'\-]{3,}$/.test(t) && /[A-ZÅÄÖ]/.test(t))
    );
  }
  ...
  for (const rawLine of lines) {
    ...
    if (!trimmed) { // blank line
      blankRun++;
      flushList();
      continue;
    }

    // If at least two blank lines before this non-empty line -> subsection heading rule
    const isSubSectionContext = blankRun >= 2 && !looksLikeHeading(trimmed);
    blankRun = 0; // reset after non-empty
    ...
    // Explicit heading (with colon or all-caps heuristic)
    if (looksLikeHeading(trimmed)) {
      blocks.push(`<h4 class='mt-4 mb-2 font-semibold text-green-400 tracking-wide'>${applyHighlight(escapeHtml(trimmed.replace(/:$/, "")))}</h4>`);
      continue;
    }

    if (isSubSectionContext) {
      // Split heading from rest by first 2+ space run
      const split = trimmed.match(/^(.+?)(\s{2,})(.+)$/);
      if (split) {
        const headingText = split[1].trim();
        const remainder = split[3].trim();
        blocks.push(`<h5 class='mt-6 mb-2 font-semibold text-green-300 tracking-wide'>${applyHighlight(escapeHtml(headingText))}</h5>`);
        if (remainder) {
          blocks.push(`<p class='mb-3 leading-relaxed text-gray-300'>${applyHighlight(escapeHtml(remainder))}</p>`);
        }
        continue;
      }
      blocks.push(`<h5 class='mt-6 mb-2 font-semibold text-green-300 tracking-wide'>${applyHighlight(escapeHtml(trimmed))}</h5>`);
      continue;
    }

    // Normal paragraph
    blocks.push(`<p class='mb-3 leading-relaxed text-gray-300'>${applyHighlight(escapeHtml(trimmed))}</p>`);
  }
  flushList();
